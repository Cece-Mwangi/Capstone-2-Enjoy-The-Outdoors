...
locationEl.onchange = onLocChange;

parkType.onchange = onParkChange;

// const tbody = document.querySelector("ParkDisplayTbl tbody");

function onLocChange() {
  const SelState = locationEl.value;

  Array.from(parkType).forEach((option) => {
    if (parkType) {
      parkType.value = "choose";
    }
  });

  const StatesSel = nationalParksArray.filter(
    (state) => state.State === SelState
  );

  const tblRow = document.getElementById("infoTbl");
  tblRow.innerHTML = "";
  //   Array.from(tblRow).forEach((state) => {
  //     tblRow.removechild(state);
  //   });

  StatesSel.forEach((state) => {
    SelLocation(tblRow, state);
  });
}
function onParkChange() {
  const typeSel = parkType.value;

  Array.from(locationEl).forEach((option) => {
    if (typeSel) {
      locationEl.value = "choose";
    }
  });

  const SelparkType = nationalParksArray.filter((park) =>
    park.LocationName.includes(typeSel)
  );

  const Tblrows = document.getElementById("infoTbl");
  Tblrows.innerHTML = "";
  //   Array.from(Tblrows).forEach((park) => {
  //     Tblrows.removeChild(park);
  //   });
  SelparkType.forEach((park) => {
    SelLocation(Tblrows, park);
  });
}

function SelLocation(myTable, myPark) {
  //   const table = document.getElementById("infoTbl");
  const row = myTable.insertRow(-1);

  const cell1 = row.insertCell(0);
  cell1.innerHTML = myPark.LocationName;

  const cell2 = row.insertCell(1);
  cell2.innerHTML = `${myPark.Address} ${myPark.City}, ${myPark.State} ${myPark.ZipCode}`;

  const cell3 = row.insertCell(2);
  cell3.innerHTML = myPark.Phone;

  const cell4 = row.insertCell(3);
  cell4.innerHTML = myPark.Fax;

  if (myPark.Address == 0) {
    cell2.innerHTML = `${myPark.City}, ${myPark.State}`;
  }
  if (cell3.innerHTML == 0) {
    cell3.innerHTML = "Information Not Available";
  }
  if (cell4.innerHTML == 0) {
    cell4.innerHTML = "Information Not Available";
  }
}
