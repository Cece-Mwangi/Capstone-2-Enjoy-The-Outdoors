...
const mountainDropdownEl = document.getElementById("SelectMtn");
mountainDropdownEl.onchange = onMountainDropdownChange;
const tbody = document.querySelector("#mountainTbl tbody");
const displayPictureDiv = document.getElementById("imgDisplay");
...
function onMountainDropdownChange() {
  ...
  const tblRow = document.getElementById("infoTbl");
  tblRow.innerHTML = "";
  ...
  selectedMountains.forEach((park) => {
    displayPicture(park);
    locationSelected(tblRow, park);
  });
}
function locationSelected(myTable, myMountain) {
  const row = myTable.insertRow(-1);

  const cell1 = row.insertCell(0);
  cell1.innerHTML = myMountain.name;
  const cell2 = row.insertCell(1);
  cell2.innerHTML = myMountain.desc;
  const cell3 = row.insertCell(2);
  cell3.innerHTML = myMountain.elevation;

  const cell4 = row.insertCell(3);
  cell4.innerHTML = myMountain.effort;
}

function displayPicture(park) {
  let imgEl = document.querySelector("#imgDisplay img");
  if (!imgEl) {
